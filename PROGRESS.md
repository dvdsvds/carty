...
- 이 리포(`carty`, Go 프로그램 본체)는 **이번에 처음 git 저장소로 초기화하고
  ...
  상태였음.
- **커스텀 프롬프트 조립 기능** 추가: 완성된 테마를 고르는 대신, 부품(경로/
  git/user@host/프롬프트 문자)을 자유 순서·개수로 골라 담고 부품마다 색을
  ...
  전체 플로우(부품 담기 → 색 선택 → 재정렬 → 적용) 끝까지 검증함 (아래
  "커스텀 프롬프트 조립" 참고).
...
zsh-theme처럼 완성된 테마 하나를 고르는 게 아니라, 프롬프트를 "부품"
단위로 자유롭게 조립하는 기능. 순서/사용 대수 완전 자유("자유 순서/개수
드래그앤드롭 조립"), 색은 부품마다 개별로 고르게(전역 팔레트 아님) —
둘 다 사용자가 명시적으로 선택한 방향. 터미널이라 진짜 드래그앤드롭은
안 되니, 부품을 고르고 커서로 위/아래 이동시키는 키보드 재배치로 구현.
...
- 새 카테고리 `zsh-custom` (`apply_method: "compose"`, 타겟
  ...
  스키마 변경 없이 그대로 통과함.
- 부품 4개(`user-host.zsh`, `path.zsh`, `git-branch.zsh`,
  `prompt-char.zsh`): 각 파일 본문에 실제 색상 hex 대신 `{{color}}`
  플레이스홀더를 넣어두고, carty가 조립 시점에 사용자가 고른 hex로
  치환. 여러 부품이 `PROMPT+="..."` 식으로 이어붙는 걸 전제로 작성함
  (oh-my-zsh 자체의 멀티라인 테마들이 쓰는 관용구와 동일) — 그래서
  어떤 부품을 어떤 순서로 골라도 항상 유효한 zsh 프롬프트가 됨.
...
- **조립 화면** (`internal/tui/compose.go`): "compose" 카테고리에
  들어가면 `screenParts`로 진입 — 왼쪽은 사용 가능한 부품 카드
  목록(라디오 아님, 중복 추가 가능), 오른쪽은 조립된 순서. `tab`으로
  두 패널 포커스 전환, 부품 추가 시 즉시 색상 피커(`screenPartColor`)
  로 넘어가서 바로 색을 고르게 함(빈 색으로 남겨두는 상태 자체가
  없음). `J`/`K`로 조립된 순서 재배치, `x`로 제거, `esc`는 "방금
  새로 추가한 부품"이면 삭제하고(취소 의미) 기존 부품 재편집 중이면
  이전 색을 그대로 유지.
- **적용 로직**: `planEntry`에 `Parts []assembledPart` 필드 추가.
  ...
  `composePartsBody`가 `PROMPT=""` 한 줄 + 부품들을 순서대로
...
- `internal/tui/compose_test.go` (영구 테스트): `{{color}}` 치환 +
  `PROMPT=""` 선두 삽입, 부품 추가 시 색상 피커로 진입하는지, 새
  부품에서 esc하면 삭제되는지, 기존 부품 재편집 중 esc하면 색이
...
- **pty 검증에서 진짜 아닌 "가짜 버그"를 두 번 겪음** — 둘 다 코드가
  ...
  2. `raw.githubusercontent.com`의 `main` 브랜치 경로가 CDN 캐시로
     ...
     본 것이었음(이중으로 헷갈림).
- 실제 pty + 개별 키 입력으로 끝까지 재현: 부품 2개 추가(각각 다른
  색 선택) → 재배치(`K`) → 적용 → `.zshrc`에 실제로 쓰인 내용까지
  확인. 두 부품이 각자 고른 색(`#003df2`, `#9af242`)으로 정확히
...
- 이 리포(`carty`)는 이번에 처음 git으로 초기화하고 GitHub에 올라간
  ...
  써봄.
- **커스텀 프롬프트 조립의 남은 한계**:
  - `resolve.DetectConflicts`가 compose 카테고리의 부품은 아예 검사
    대상에 안 넣음 (각 부품이 `provides.functions`를 선언 안 해서
    지금은 문제없지만, 나중에 부품이 전역 함수를 정의하게 되면 충돌
    검사가 비어있는 채로 지나감).
  - 부품 목록 화면(`viewParts`)엔 스크롤이 없음 — 지금 부품이 4개뿐이라
    문제없지만, 부품이 많아지면 항목 목록에서 고친 것과 같은 문제가
    다시 생길 수 있음.
  - 같은 카테고리 안에서 "완성된 테마"(`zsh-theme`)와 "커스텀 조립"
    (`zsh-custom`)을 동시에 켜면 둘 다 `PROMPT=...`/`PROMPT+=...`를
    건드리므로 서로 간섭할 수 있음 — 지금은 서로 다른 마커 구역이라
    막지 않고, 알려진 한계로만 남겨둠.
  - fish용 조립 기능은 없음 (zsh만).
